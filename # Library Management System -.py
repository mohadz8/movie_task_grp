...
books = {
    "1": {"title": "Harry Potter and the Sorcerer's Stone", "copies": 5},
    "2": {"title": "The Lord of the Rings", "copies": 3},
    "3": {"title": "The Hobbit", "copies": 2},
    "4": {"title": "1984", "copies": 4}
}
...
def register_user():
    """
    Task 1: Register a User
    Prompts for name, age, and membership type.
    Validates age >= 12 and membership type is 'digital' or 'physical'.
    Returns a user profile dictionary and adds it to the users list.
    """
...
def borrow_book():
    ...
    Prompts for user name and book ID.
    Validates user exists, book exists, and availability.
    Deducts one copy and appends book ID to user's borrowed list.
    """
...
def return_book():
    """
    Return Book Flow (similar to borrow, but adds copy back).
    Prompts for user name and book ID.
    Validates user exists and has borrowed the book.
    Adds one copy back and removes from user's borrowed list.
    """
    user_name = input("Enter user name: ").strip()
    user = next((u for u in users if u["name"] == user_name), None)
    if not user:
...
def view_member():
    """
    View Member Details
    Prompts for user name and displays their profile.
    """
    user_name = input("Enter user name: ").strip()
    user = next((u for u in users if u["name"] == user_name), None)
    if not user:
...
def main_menu():
    """
    Task 3: Main Menu
    Loops through menu options, calling appropriate functions.
    """
    while True:
